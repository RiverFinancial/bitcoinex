...
- PSBT decoding also rejects trailing bytes *inside* a value payload — a `witness_utxo` with bytes after the scriptPubKey (`:invalid_witness_utxo`), a `final_scriptwitness` with bytes after the last stack item (`:invalid_final_scriptwitness`), and an `xpub`/`bip32_derivation` value whose path bytes are not whole 32-bit indexes (`:invalid_derivation`). These were previously accepted and the extra bytes silently dropped on re-serialize, breaking losslessness.
