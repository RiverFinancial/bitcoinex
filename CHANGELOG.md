...
## [Unreleased]
### Fixed
- **Breaking:** `Script.to_address/2` now requires the script to match a standard template *exactly*. It previously pattern-matched only the leading items and discarded the rest, so a non-standard script that merely shared a prefix with a template — `OP_DUP OP_HASH160 <20B>` with no `OP_EQUALVERIFY OP_CHECKSIG`, `OP_HASH160 <20B> OP_1`, `OP_0 <20B> OP_1`, or any of these with extra trailing opcodes — encoded to the address of the *canonical* template script, which does not encumber the same coins. Funds sent to such an address land on a scriptPubKey different from the one that was inspected. These scripts now return `{:error, "non standard script type"}`, matching what `get_script_type/1` already reported for them (`:non_standard`). A witness v0 program of a length other than 20 or 32 now returns that same error instead of `"invalid witness program length..."`.
- `ExtendedKey.parse_extended_key/1` returns `{:error, "invalid public key"}` for a checksum-valid xpub whose 33-byte key field is not a parseable point. `check_point/1` asserted `{:ok, pubkey} = Point.parse_public_key(key)`, so a key field with a prefix byte other than `0x02`/`0x03` raised `MatchError` — reachable from PSBT decoding, where the extended key comes from caller-supplied bytes.
- `Secp256k1.Point.parse_public_key/1` rejects non-canonically encoded coordinates: a compressed key with `x >= p`, or an uncompressed key with `x >= p` or `y >= p`. `x >= p` was silently reduced mod p by `get_y/2`, giving a point two valid serializations (xpub/pubkey malleability). This mirrors the bound `lift_x/1` already enforced.
- `Secp256k1.Point.parse_public_key/1` returns `{:error, "invalid public key"}` instead of raising for a binary that is neither 33/65 raw bytes nor a 66/130-character hex string. The hex fallthrough did `String.downcase() |> Base.decode16!()`, which raised `ArgumentError` on non-hex input and `FunctionClauseError` on a non-UTF-8 binary.
- **Breaking:** `ExtendedKey.derive_extended_key/2` returns `{:error, "wildcard cannot be derived"}` for a derivation path containing a wildcard. `:any` (from `*`) previously returned the key derived so far, silently discarding the remainder of the path — so `m/0/*` yielded exactly the `m/0` key, reusing one key across every logical sub-account it was meant to range over — while `:anyh` (from `*'`/`*h`) fell through to integer derivation and surfaced an unrelated `"...not an integer..."` message. A wildcard names a set of children, not one child; both now error identically. Enumerating a wildcard's children is the caller's job: substitute concrete indexes into the path.
