...
@spec divide(byte, byte) :: byte         # a / b; raises ArithmeticError on b == 0
@spec pow(byte, integer) :: byte
@spec interpolate(points :: [{byte, binary}], x :: byte) :: binary
```

**Naming: `divide/2`, not `div/2`.** Defining a local `div/2` and calling it anywhere in the same module (e.g. from `interpolate/2`) is a hard compile error — `error: imported Kernel.div/2 conflicts with local function` (verified on Elixir 1.18.1/OTP 27). `import Kernel, except: [div: 2]` would work but silently changes the meaning of integer `div` in the module; `divide/2` avoids the trap outright.
...
**Options considered.** (A) Compile-time tables memoized into module attrs. (B) Runtime tables in an Agent/ETS. **Decision: A** — matches the repo's compile-time `@base58_decode_map` / `@gen` style; no runtime process (the library has no supervision tree). Runtime `mul/2` uses the log tables.

**Compile-time mechanics (important).** A `defp mul_slow/2` **cannot** be called from a module attribute in the same module — functions aren't callable at that module's own compile time (`error: undefined function mul_slow/2`, verified on Elixir 1.18.1). Generate the tables in the **module body** with plain data-flow instead: an `Enum.map_reduce(0..254, 1, fn _, x -> {x, slow_mul.(x, 3)} end)` over an anonymous-function shift-and-reduce (`slow_mul` closes over itself via an extra arg, or is written as an inline `Enum.reduce` over the 8 bits of `b` with `0x11B` reduction), then `@exp_table List.to_tuple(exp_list)` / `@log_table` built from it. This is verified to compile; no helper module or macro needed.
...
**`split_secret/4`.**
- `threshold == 1` → return `[{0, secret}, {1, secret}, …, {count-1, secret}]` (all equal). Note this multi-copy case is reachable: SLIP-39 prohibits `t == 1 && n > 1` only at the **member** level; a **group** threshold of 1 with several groups is legal, and every group share is then the EMS verbatim.
...
- **Decode:** with `value_words = word_count - 7`, `value_bits = value_words * 10` and `pad_bits = rem(value_bits, 16)`. Per SLIP-39, `pad_bits` **MUST NOT exceed 8** — reject with `{:error, :invalid_mnemonic_length}` if `pad_bits > 8` (this rules out e.g. 21-word mnemonics, where `rem(140, 16) == 12`; the reference implementation reports this the same way as a too-short mnemonic, "Invalid mnemonic length", and official vector #40, labelled "invalid master secret length", is exactly this 21-word case) and `{:error, :invalid_padding}` if any padding bit is nonzero. `share_value` is the trailing `value_bits - pad_bits` bits (a whole number of bytes by construction).
...
`encode/1` cannot fail on a well-formed struct (all fields are range-constrained and `share_value` is whole bytes), so it returns a plain string — matching `ExtendedKey.serialize_extended_key/1`, which returns a bare binary. Struct validity is guaranteed by construction in `generate_mnemonics/4` / `decode/1`.

`decode/1`: split words → indices (`words_to_indices`) → verify RS1024 (`{:error, :invalid_checksum}`) and word count ≥ `@min_mnemonic_length_words` (`{:error, :invalid_mnemonic_length}`) → strip 3 checksum words → bit-unpack metadata → validate padding → validate `group_threshold ≤ group_count` (`{:error, :invalid_group_threshold}` — official vectors #10/#29 exercise exactly this at the single-share level) → build struct (adding 1 to the stored thresholds/counts). `encode/1`: reverse, appending `rs1024_create_checksum`.
...
### 3.10 Shared helpers (DRY)

**Bit packing.** BIP-39 (11-bit) and SLIP-39 (10-bit) both convert between a bitstring and a list of fixed-width indices. Factor the *generic* direction that isn't a trivial comprehension into `Utils`:

```
@spec int_list_to_bits([non_neg_integer], width :: pos_integer) :: bitstring
def int_list_to_bits(ints, width)   # for i <- ints, into: <<>>, do: <<i::size(width)>>
```

The reverse direction is the one-liner `for <<i::size(width) <- bits>>, do: i` and stays inline in each module (a shared function buys nothing there).

**Wordlist loading.** BIP-39 and SLIP-39 both need "read `priv/*.txt` at compile time → `{tuple, %{word => index}}`". A plain function in a separate module **is** callable at another module's compile time (unlike a same-module `defp`, see §3.4), so share it as `lib/wordlist.ex` — `Bitcoinex.Wordlist`:

```
@spec load!(path :: Path.t(), expected_count :: pos_integer) ::
        {words :: tuple(), index :: %{String.t() => non_neg_integer()}}
def load!(path, expected_count)   # File.read! -> String.split -> assert count -> {List.to_tuple, Map}
```

Callers (`BIP39`, `SLIP39.Encoding`) each do:

```
@wordlist_path Path.join(__DIR__, "../priv/bip39_english.txt")   # __DIR__-relative, NOT :code.priv_dir (app not loaded at compile time)
...
`@external_resource` makes `mix compile` re-track the `.txt` files; `expected_count` makes a truncated/corrupted word list a **compile** failure, not a runtime mis-encode. **Options considered.** (A) Duplicate the 4-line load in each module. (B) A `use Bitcoinex.Wordlist` macro. (C) Plain `load!/2` function. **Decision: C** — DRY without macro indirection (the repo has no macro precedent), and centralizes the count/uniqueness validation.

Word lists and checksum logic otherwise stay scheme-specific (RS1024 ≠ bech32 polymod constants; not worth unifying).
...
### `lib/wordlist.ex` — `Bitcoinex.Wordlist` (new)
- `load!/2` (§3.10) — compile-time word-list loader with count/uniqueness validation. Raises on missing file, wrong count, or duplicate words (compile-time programmer error, not a result tuple).
...
- `add/2`, `mul/2`, `divide/2`, `pow/2`, `interpolate/2` (§3.4).
...
- Struct (§3.8), `encode/1`, `decode/1`. Error atoms: `:invalid_checksum`, `:invalid_mnemonic_length` (too-short mnemonic, or `pad_bits > 8` as in official vector #40), `:invalid_padding`, `:invalid_group_threshold`.
...
Test files mirror `lib/` as `test/*_test.exs` / `test/slip39/*_test.exs`, `use ExUnit.Case`, `doctest` the module, hex via `Base.decode16!(s, case: :lower)` — matching `extendedkey_test.exs` / `base58_test.exs`. This is security-critical code: every official vector is exercised, plus property/fuzz coverage (§8.1).

### Official vectors (already vendored under `test/data/`)

| File | Source | Contents | SHA-256 |
|------|--------|----------|---------|
| `test/data/bip39_vectors.json` | [trezor/python-mnemonic `vectors.json`](https://raw.githubusercontent.com/trezor/python-mnemonic/master/vectors.json), `"english"` key extracted (upstream full-file sha256 `fa3b937b7cff9c9b8ecd3aa011faeb8d6dd67993174b72326e83f4de8fdb30f8`) | 24 vectors: `[entropy_hex, mnemonic, seed_hex, xprv]`, seeds derived with passphrase `"TREZOR"` | `7dc2801c307616b363acb80a0a22f9ce74a9697ebbaf78bcba3f7fab2745d7f8` |
| `test/data/slip39_vectors.json` | [trezor/python-shamir-mnemonic `vectors.json`](https://raw.githubusercontent.com/trezor/python-shamir-mnemonic/master/vectors.json), byte-identical | 45 vectors: `[description, mnemonics, ms_hex, xprv]` — 15 valid (non-empty `ms_hex`; recovery uses passphrase `"TREZOR"`; `xprv` = `seed_to_master_private_key(ms)`, an end-to-end hook into `ExtendedKey`) and 30 invalid (empty `ms_hex`; each description names the failure mode, mapping 1:1 onto our error atoms — invalid checksum/padding, insufficient shares, mismatched id/e/thresholds/counts, duplicate member index, invalid digest, `GT > G`, insufficient length, invalid MS length) | `13ebecebdd869dd2bc2cdf69e7ce3a158cf106cac76c39d17682b1c6cdabbdc4` |

**Storage decision (resolves OQ):** inline `@`-attribute vectors where small, file where very large. BIP-39's 24 english vectors are inlined as `@bip39_vectors` in `test/bip39_test.exs` at implementation time (transcribed from the vendored JSON, which is then deleted in PR B2; the test file comment records the source URL + upstream sha256) — matching the `extendedkey_test.exs` inline-BIP32-vector precedent. SLIP-39's 45 multi-mnemonic vectors (22 KB) are *very large* → stay in `test/data/slip39_vectors.json`, parsed at test time with Elixir 1.18's built-in `JSON.decode!/1` (no new dependency; the `mix.exs` `files` list already ships `test`).

### 8.1 Property & fuzz tests (`stream_data`, already a test dep)

`use ExUnitProperties` (precedent: imported-but-unused in `base58_test.exs`; these are the repo's first real `property` blocks). Each property runs the default 100 generated cases; deterministic — no CSPRNG in generators (use `StreamData` seeds), test `rng` for SLIP-39 generation is a seeded deterministic stub.

- **GF256 algebra** (`gf256_test.exs`): ∀ `a,b ∈ 0..255`: `mul(a,b) == mul(b,a)`; `b != 0 → divide(mul(a,b), b) == a`; `mul(a, add(b,c)) == add(mul(a,b), mul(a,c))` (distributivity); `pow(a, 255) == 1` for `a != 0` (Fermat). These catch table-generation errors that single known-answer tests can miss.
- **Shamir round-trip** (`shamir_test.exs`): ∀ random secret (16/32 bytes), random `2 ≤ T ≤ N ≤ 16`, random `T`-subset of the `N` shares: `recover_secret(T, subset) == {:ok, secret}`. And: corrupting one random byte of one share in the subset → `{:error, :invalid_digest}`.
- **Cipher round-trip** (`cipher_test.exs`): ∀ random even-length MS (16–32 bytes), random printable-ASCII passphrase, `e ∈ 0..2`, `ext ∈ {true,false}`, random id: `decrypt(encrypt(ms, ...), ...) == ms`.
- **Share codec round-trip** (`share_test.exs`): ∀ random valid `Share` struct (all fields drawn from their ranges, `share_value` of 16 or 32 random bytes): `decode(encode(share)) == {:ok, share}`.
- **BIP-39 round-trip** (`bip39_test.exs`): ∀ random entropy of each valid length: `entropy_to_mnemonic |> mnemonic_to_entropy == {:ok, entropy}`.
- **Full-stack round-trip** (`slip39_test.exs`): ∀ random MS, random valid group configuration (drawn small: `GT ≤ G ≤ 4`, `Ti ≤ Ni ≤ 4` to bound PBKDF2 cost — use `iteration_exponent: 0`), exact-threshold subsets: `generate_mnemonics |> combine_mnemonics == {:ok, ms}`; and share **order independence** (shuffled mnemonic list → same result).
- **Decoder never raises (fuzz)**: ∀ arbitrary `String.t()` (printable and non-printable, random word counts, valid words in random order): `BIP39.mnemonic_to_entropy/1`, `SLIP39.Share.decode/1`, and `SLIP39.combine_mnemonics/2` return `{:ok, _} | {:error, atom}` — never an exception. Generators: fully random strings; random sequences of *valid* wordlist words (exercises past the word-lookup layer into checksum/bit-unpack); valid mnemonics with 1–3 random word substitutions.
- **Checksum detection guarantees**: SLIP-39 — ∀ valid share mnemonic, mutating any 1, 2, or 3 words → `Share.decode/1` returns an error (RS1024 *guarantees* detection ≤ 3 words; assert the guarantee, not just probability). BIP-39 — a 1-word substitution passes the (ENT/32-bit) checksum with probability up to 2⁻⁴, so assert the weaker true property: decode never returns the *original* entropy (and count that most cases error, without asserting all do).

### 8.2 Unit tests to add (exhaustive enumeration)

**`test/utils_test.exs`** (new file — Utils is currently untested)
- `pbkdf2/5`: assert against a known BIP-39 seed vector and a known SLIP-39 round-function output.
- `int_list_to_bits/2`: packs `[1,2,3]` at width 11 to the expected bitstring; round-trips against the `for <<i::11 <- bits>>` comprehension.

**`test/wordlist_test.exs`**
- `load!/2` on each `priv/` list: BIP-39 has exactly 2048 entries, SLIP-39 exactly 1024; all unique; index map inverts the tuple (`elem(words, index[w]) == w` for all words).
- SLIP-39 list constraints: every word length 4–8, 4-letter prefixes unique, alphabetically sorted (guards against a corrupted/reordered vendored file).
- `load!/2` raises on wrong `expected_count`.
...
- `divide/2`: `mul(a, divide(1,a)) == 1` for all `a ∈ 1..255`; `divide(a,b)` inverse of `mul` on samples; `divide(a, 0)` raises `ArithmeticError`.
...
- `decode/1`: non-zero padding bits → `{:error, :invalid_padding}`; 21-word mnemonic (`rem(140,16) == 12 > 8`) → `{:error, :invalid_mnemonic_length}` (see §3.8).
...
- `combine_mnemonics/2` on all 15 valid `test/data/slip39_vectors.json` entries (passphrase `"TREZOR"`) → expected MS hex, **and** `ExtendedKey.seed_to_master_private_key(ms)` → the vector's `xprv` (end-to-end into BIP-32).
- All 30 invalid vectors → `{:error, _}`; for each, assert the *specific* atom its description implies (`:invalid_checksum`, `:invalid_padding`, `:insufficient_member_shares` for lone 2-of-3 shares, `:mismatching_shares`, `:invalid_group_threshold`, `:duplicate_member_index`, `:invalid_digest`, `:insufficient_groups`, `:invalid_mnemonic_length`), not just any error.
- `generate_mnemonics/4` / `combine_mnemonics/2` with non-printable-ASCII passphrase (e.g. `"café"`, `<<7>>`) → `{:error, :invalid_passphrase}`.
...
### Toolchain / fixtures
- Add `priv/` word lists before compiling (module attrs read them at compile time — a missing file fails compilation loudly; `Wordlist.load!/2` count checks make truncation a compile error).
- Official vectors are **already vendored** at `test/data/{bip39,slip39}_vectors.json` (provenance + hashes in the table above). SLIP-39's is parsed at test time via Elixir 1.18's built-in `JSON.decode!/1`; BIP-39's is transcribed to inline `@bip39_vectors` in PR B2 and the JSON deleted.
- Property tests use the existing `stream_data` dep; keep generated SLIP-39 configs small (`iteration_exponent: 0`, groups ≤ 4) so PBKDF2 keeps the suite fast.
...
- **PR B0 — `S0:` word lists & packaging.** Add `priv/bip39_english.txt`, `priv/slip39_english.txt`, `lib/wordlist.ex` (`Wordlist.load!/2`), and `test/wordlist_test.exs`; add `priv` to `mix.exs` package `files`. Blocks B2, S5.
- **PR B1 — `S1:` shared primitives.** `Utils.pbkdf2/5`, `Utils.int_list_to_bits/2`, `test/utils_test.exs`. Independent. Blocks B2, S3.
- **PR B2 — `S2:` BIP-39.** `lib/bip39.ex`, `test/bip39_test.exs` (inlines the 24 english vectors from `test/data/bip39_vectors.json`, then deletes that JSON), `CHANGELOG.md`. Depends on B0, B1. Independently shippable (delivers "mnemonic → xprv").
- **PR S3 — `S3:` SLIP-39 field & sharing.** `lib/slip39/gf256.ex`, `lib/slip39/shamir.ex` + tests. Depends on B1 (sequence after B1 to avoid `Utils` merge conflicts).
...
- **PR S6 — `S6:` SLIP-39 orchestrator.** `lib/slip39.ex`, `test/slip39_test.exs` + `test/data/slip39_vectors.json` (already vendored), `CHANGELOG.md` + `mix.exs` version bump. Depends on S3, S4, S5.
...
- **Passphrase-wrong on `combine_mnemonics`** — *resolved 2026-07-01*: the caller is responsible; SLIP-39 by design cannot detect a wrong passphrase (plausible deniability), so `combine_mnemonics/2` returns `{:ok, ms}` for any valid share set regardless of passphrase. No fingerprint check. Documented in the `@doc`.
- **Vector storage location** — *resolved 2026-07-01*: inline `@` attributes unless very large. BIP-39 english (24 vectors) → inline; SLIP-39 (45 vectors, 22 KB) → `test/data/slip39_vectors.json` read via built-in `JSON`. See §8.
