defmodule Bitcoinex.PSBT do
  ...
  defp parse(<<@magic :: big-size(32), @separator :: big-size(8),  psbt :: binary >>) do
    ...
    {outputs, psbt} = Out.parse_outputs(psbt, out_counter)
    
    Logger.info("#{inspect(global)}")
    Logger.info("#{inspect(inputs)}")
    Logger.info("#{inspect(psbt)}")
...
defmodule Bitcoinex.PSBT.Global do
    ...
    defp parse(<<@psbt_global_xpub :: big-size(8) >>, psbt, global) do
        {value, psbt} = PsbtUtils.parse_compact_size_value(psbt)
        global = %Global{global | xpub: Base.encode16(value, case: :lower)}
        {global, psbt}
    end
...
defmodule Bitcoinex.PSBT.In do
    ...
    defp parse_input(psbt, inputs, num_inputs) do
        {input, psbt} = PsbtUtils.parse_key_value(psbt, %In{}, &parse/3)
    ...
    defp parse(<<@psbt_in_partial_sig :: big-size(8), public_key :: binary-size(33) >>, psbt, input) do
        {value, psbt} = PsbtUtils.parse_compact_size_value(psbt)
        input = %In{input | partial_sig: %{public_key: public_key, signature: value}}
        {input, psbt}
    end
    ...
    defp parse(<<@psbt_in_redeem_script :: big-size(8)>>, psbt, input) do
        {value, psbt} = PsbtUtils.parse_compact_size_value(psbt)
        input = %In{input | redeem_script: value}
        {input, psbt}
    end

    defp parse(<<@psbt_in_witness_script :: big-size(8)>>, psbt, input) do
        {value, psbt} = PsbtUtils.parse_compact_size_value(psbt)
        input = %In{input | witness_script: value}
        {input, psbt}
    end

    defp parse(<<@psbt_in_bip32_derivation :: big-size(8), public_key :: binary-size(33) >>, psbt, input) do
        {value, psbt} = PsbtUtils.parse_compact_size_value(psbt)
        input = %In{input | bip32_derivation: [ input.bip32_derivation | %{public_key: Base.encode16(public_key, case: :lower), derivation: value}]}
        {input, psbt}
    end

    defp parse(<<@psbt_in_final_scriptsig :: big-size(8)>>, psbt, input) do
        {value, psbt} = PsbtUtils.parse_compact_size_value(psbt)
        input = %In{input | final_scriptsig: value}
        {input, psbt}
    end
    ...
    defp parse(<<@psbt_in_final_scriptwitness :: big-size(8) >>, psbt, input) do
        {value, psbt} = PsbtUtils.parse_compact_size_value(psbt)
        out = Witness.witness(value)
        input = %In{input | final_scriptwitness: out}
        {input, psbt}
    end
end
...
defmodule Bitcoinex.PSBT.Out do
    ...
    defp parse(<<@psbt_out_bip32_derivation :: big-size(8), public_key :: binary-size(33)>>, psbt, output) do
        {value, psbt} = PsbtUtils.parse_compact_size_value(psbt)
        # todo separate struct for bip32 derivation
        output = %Out{output | bip32_derivation: [ output.bip32_derivation | %{public_key: Base.encode16(public_key, case: :lower), derivation: value}] }
        {output, psbt}
    end
end
