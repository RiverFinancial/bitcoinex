...
defmodule Bitcoinex.Transaction do
    ...
    defp parse(tx_bytes) do
        ...
        {inputs, remaining} = In.inputs(in_counter, remaining)

        # Outputs.
        {out_counter, remaining} = TxUtils.get_counter(remaining)
        {outputs, remaining} = Out.outputs(out_counter, remaining)
    
        # If flag 0001 is present, this indicates an attached segregated witness structure.
        {witnesses, remaining} =
        if is_segwit == :segwit do
            Witness.witness(in_counter, remaining)
        else
            {nil, remaining}
        end 

        << lock_time :: little-size(32), _  :: binary >> = remaining
...
defmodule Bitcoinex.Transaction.Witness do 
    ...
    def witness(0, remaining), do: {nil, remaining}

    def witness(counter, witnesses) do
    ...
    defp parse(remaining, witnesses, count) do 
        ...
        {witness, remaining} = 
            if stack_size == 0 do
                ...
                {stack_items, remaining} = parseWitness(remaining, [], stack_size)
    ...
    defp parseWitness(remaining, stack_items, 0), do: {Enum.reverse(stack_items), remaining}

    defp parseWitness(remaining, stack_items, stack_size) do
        {item_size, remaining} = TxUtils.get_counter(remaining)

        << stack_item :: binary-size(item_size), remaining :: binary >> = remaining

        parseWitness(remaining, [Base.encode16(stack_item, case: :lower) | stack_items], stack_size - 1)
    end
end
...
defmodule Bitcoinex.Transaction.In do
    ...
    def inputs(counter, inputs) do
        parse(inputs, [], counter)
    end

    defp parse(remaining, inputs, 0), do: {Enum.reverse(inputs), remaining}

    defp parse(<<prev_txid :: binary-size(32), prev_vout :: little-size(32), remaining :: binary >>, inputs, count) do
         
        {script_len, remaining} = TxUtils.get_counter(remaining)
        
        << script_sig :: binary-size(script_len), sequence_no::little-size(32), remaining :: binary >> = remaining
        
        input = %In{
            prev_txid: Base.encode16(<< :binary.decode_unsigned(prev_txid, :big) ::  little-size(256) >> , case: :lower),
            prev_vout: prev_vout,
...
defmodule Bitcoinex.Transaction.Out do
    ...
    def outputs(counter, outputs) do
